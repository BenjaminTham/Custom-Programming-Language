...
export default class Parser {
	...
	private parse_stmt(): Stmt {
		...
		switch (this.at().type) {
			...
			case TokenType.If:               // <-- new
      			return this.parse_if_statement();
			case TokenType.While:                // <-- new case
	...
	private parse_comparison_expr(): Expr {
		// Start with an additive expression
		...
		while (this.at().type === TokenType.BinaryOperator 
		  ...
		  const operator = this.eat().value; // e.g. "<"
	...
	private parse_primary_expr(): Expr {
      ...
      case TokenType.String: {
        const stringToken = this.eat(); // this.eat() consumes one token
        return {
          kind: "StringLiteral",
	...
	private parse_if_statement(): Stmt {
		...
		// 2) parse condition in parentheses => if ( condition ) 
		...
		this.expect(TokenType.OpenBrace, "Expected '{' after if(...)");
		...
		this.expect(TokenType.CloseBrace, "Missing closing brace '}' after if block");
	  
		// 4) optionally parse 'else' branch
		let elseBranch: Stmt[] | undefined = undefined;
		if (this.at().type === TokenType.Else) {
		  // consume the 'else' token
		  this.eat();
	  
		  // there are a few styles: if the else is "else if", or normal "else { ... }"
		  // We'll keep it simple and assume a block after 'else'
	  
		  // if there's a brace => else { ... }
		  if (this.at().type === TokenType.OpenBrace) {
			this.eat(); // consume '{'
			elseBranch = [];
			while (
			...
			this.expect(TokenType.CloseBrace, "Missing closing brace '}' after else block");
		  } else {
			// If you also want "else if" chain, you could detect if TokenType.If 
			// and recursively parse another parse_if_statement. For now, let's just 
			// parse a single statement:
			elseBranch = [ this.parse_stmt() ];
		  }
		}
	  
		// Build and return an IfStmt node
		return {
		  kind: "IfStmt",
		  condition,
		  thenBranch,
	  ...
	  private parse_while_statement(): Stmt {
		// 1) Consume "while" token
		this.eat(); // 'while'
	  
		// 2) Parse condition in parentheses => while( condition )
		...
		this.expect(TokenType.OpenBrace, "Expected '{' after while(...)");
		const body: Stmt[] = [];
		...
		this.expect(TokenType.CloseBrace, "Missing '}' after while body");
	  
		// 4) Build and return WhileStmt node
		return {
		  kind: "WhileStmt",
		  condition,
		  body,
